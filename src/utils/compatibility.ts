...
function filterIgnoredBrowsers(
  unsupportedBrowsers: string[],
  ignoreBrowsers: string[] = []
): string[] {
  if (ignoreBrowsers.length === 0) return unsupportedBrowsers;
  ...
  return unsupportedBrowsers.filter((browser) => {
    return !ignoreBrowsers.some((ignored) => {
      ...
      // Exact match
      if (normalizedBrowser === normalizedIgnored) return true;

      // Browser name match (e.g., "ie" matches "ie 11")
      if (normalizedBrowser.startsWith(`${normalizedIgnored} `)) return true;
...
export function checkHtmlElementCompatibility(
  ...
  const filteredUnsupportedBrowsers = filterIgnoredBrowsers(
    unsupportedBrowsers,
    ignoreBrowsers
  );
...
export function checkHtmlElementDeprecation(
  element: string
): DeprecationResult {
  const feature = `html.elements.${element}`;
  const elementData = bcd.html?.elements?.[element];
  ...
  if (status?.deprecated) {
    return {
      isDeprecated: true,
      deprecationNote:
        status.deprecated === true ? undefined : String(status.deprecated),
      feature,
      mdnUrl: generateMdnUrl(feature),
    };
  }
